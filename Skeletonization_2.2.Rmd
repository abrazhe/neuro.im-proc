...
```{python}
#tips = [t for t in tqdm(all_tips) if t in domain_outer_shell_pts]
...
```{python}
nx.set_node_attributes(gx_all, get_attrs_by_nodes(graph, qstack), 'occurence')
```

```{python}
n = list(gx_all.nodes())[0]
gx_all.nodes[n]
```
...
```{python}
nx.set_node_attributes(high_occurence_graph1b, get_attrs_by_nodes(graph, sigma_arr, lambda x: id2sigma[x]), 'sigma_mask')

nx.set_node_attributes(high_occurence_graph1b, get_attrs_by_nodes(graph, sigma_sato, lambda x: id2sigma[x]), 'sigma_opt')

nx.set_node_attributes(high_occurence_graph1b, get_attrs_by_nodes(graph, qstack), 'occurence')
```
...
colored_paths_high_occ = paths_to_colored_stack(xpaths_high_occ, final_image.shape, change_color_at_branchpoints=False)
```

```{python}
xpaths_high_occ1 = graph_to_paths(high_occurence_graph1b)
colored_paths_high_occ1 = paths_to_colored_stack(xpaths_high_occ1, final_image.shape, change_color_at_branchpoints=False)
```

```{python}
colored_paths_high_occ2 = paths_to_colored_stack(graph_to_paths(high_occurence_large_sigma_graph,), final_image.shape, change_color_at_branchpoints=False)
colored_paths_high_occ3 = paths_to_colored_stack(graph_to_paths(high_occurence_large_opt_sigma_graph,), final_image.shape, change_color_at_branchpoints=False)
```
...
```{python}
if verbose:
    w2 = napari.view_image(final_image, ndisplay=3, opacity=0.5)
    #props = {'path-id': ['line'+str(i) for i in np.arange(len(xpaths))]}
    w2.add_image(colored_paths_all,  channel_axis=3, colormap=['red','green','blue'], name='cp_all')
```
