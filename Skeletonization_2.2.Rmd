...
```{python}
nx.set_node_attributes(graph, get_attrs_by_nodes(graph, sigma_arr, lambda x: id2sigma[x]), 'sigma_mask')
```

```{python}
nx.set_node_attributes(graph, get_attrs_by_nodes(graph, sigma_sato, lambda x: id2sigma[x]), 'sigma_opt')
```
...
```{python}
g = nx.parse_gml("\n".join(nx.generate_gml(Gtest, tup2str)), destringizer=str2tup)
```
