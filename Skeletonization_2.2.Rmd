...
```{python}
high_occurence_tips = get_tips(high_occurence_graph1) # Кончики веток из предыдущего графа
...
```{python}
xpaths_high_occ1 = graph_to_paths(thin_graph)
...
w = napari.view_image(img, opacity=0.5, ndisplay=3)
...
```{python}
# %time xpaths_all = graph_to_paths(filter_graph(gx_all, lambda node: node['sigma_mask'] >= sigmas[-5]))
```
