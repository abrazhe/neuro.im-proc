...
Если получится выделять множство узлов не через for, с помощью маски, то можно переделать функцию получше
...
def set_graph_area(G, area_mask, out=True, set_name=None):
    ...
    node_coords = np.array(nodes.keys())
    for node, _ in nodes:
        if area_mask[tuple(node)] and not have key area:
            if set_name is not None:
                nx.set_node_attributes(G, {node: {'area':set_name}})
    if out:
        return G.subgraph_view()
