...
idx_lu = idxs[rd_crop].reshape(((shape[0]-1)*(shape[1]-1), 2))
idx_rd = idxs[lu_crop].reshape(((shape[0]-1)*(shape[1]-1), 2))

idx_uu = idxs[dd_crop].reshape(((shape[0]-1)*(shape[1]), 2))
idx_dd = idxs[uu_crop].reshape(((shape[0]-1)*(shape[1]), 2))

idx_ru = idxs[ld_crop].reshape(((shape[0]-1)*(shape[1]-1), 2))
idx_ld = idxs[ru_crop].reshape(((shape[0]-1)*(shape[1]-1), 2))

idx_ll = idxs[rr_crop].reshape(((shape[0]-1)*(shape[1]), 2))
idx_rr = idxs[ll_crop].reshape(((shape[0]-1)*(shape[1]), 2))
```

```{python}
for i, j in zip(idx_ru, idx_ld):
    print(i, j)
```

```{python}
weights = 1 - weight_of_arr(arr, lu)
weight = weights[lu_crop]
```

```{python}
weight[weight < 0.7], tuple(np.asarray(idx_lu)[weight.ravel()<0.7][0])
```

```{python}
def calc_edges(arr1, arr2, crop, index1, index2):
    weights = 1 - weight_of_arr(arr1, arr2)
    weight = weights[crop].ravel()
    idx1 = [(j, i) for i, j in index1[weight<0.7]]
    idx2 = [(j, i) for i, j in index2[weight<0.7]]
    return zip(idx1, idx2, weight[weight<0.7])
```
...
# %time Gh.add_weighted_edges_from(calc_edges(lu, arr, lu_crop, idx_lu, idx_rd))
...
# %time Gh.add_weighted_edges_from(calc_edges(uu, arr, uu_crop, idx_uu, idx_dd))
...
# %time Gh.add_weighted_edges_from(calc_edges(ru, arr, ld_crop, idx_ru, idx_ld))
...
# %time Gh.add_weighted_edges_from(calc_edges(ll, arr, ll_crop, idx_ll, idx_rr))
...
```{python}
1-weight_of_arr(uu, arr), (1-weight_of_arr(dd, arr))[dd_crop]
```
...
```{python}
napari.view_image(final_image)
```

Что-то не так с матрицами и узлами. Есть смещение по индексам. Пока непонятно почему

```{python}
path = nx.shortest_path(Gh, (105, 124), (159, 143), weight='weight')
```
...
rro_crop = (slice(1, None), slice(None), slice(None, -1))
llo_crop = (slice(1, None), slice(None), slice(1, None))
...
rrr_crop = (slice(None), slice(None), slice(None, -1))
lll_crop = (slice(None), slice(None), slice(1, None))
...
rri_crop = (slice(None, -1), slice(None), slice(None, -1))
lli_crop = (slice(None, -1), slice(None), slice(1, None))
...
```{python}
idx_ooo = idxs[ooo_crop].reshape(((shape[0]-1)*(shape[1])*(shape[2]), 3))

idx_luo = idxs[luo_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]-1), 3))
idx_rdo = idxs[rdo_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]-1), 3))

idx_uuo = idxs[uuo_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]), 3))
idx_ddo = idxs[ddo_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]), 3))

idx_ruo = idxs[ruo_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]-1), 3))
idx_ldo = idxs[ldo_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]-1), 3))

idx_llo = idxs[llo_crop].reshape(((shape[0]-1)*(shape[1])*(shape[2]-1), 3))
idx_rro = idxs[rro_crop].reshape(((shape[0]-1)*(shape[1])*(shape[2]-1), 3))

idx_luu = idxs[luu_crop].reshape(((shape[0])*(shape[1]-1)*(shape[2]-1), 3))
idx_rdd = idxs[rdd_crop].reshape(((shape[0])*(shape[1]-1)*(shape[2]-1), 3))

idx_uuu = idxs[uuu_crop].reshape(((shape[0])*(shape[1]-1)*(shape[2]), 3))
idx_ddd = idxs[ddd_crop].reshape(((shape[0])*(shape[1]-1)*(shape[2]), 3))

idx_ruu = idxs[ruu_crop].reshape(((shape[0])*(shape[1]-1)*(shape[2]-1), 3))
idx_ldd = idxs[ldd_crop].reshape(((shape[0])*(shape[1]-1)*(shape[2]-1), 3))

idx_lll = idxs[lll_crop].reshape(((shape[0])*(shape[1])*(shape[2]-1), 3))
idx_rrr = idxs[rrr_crop].reshape(((shape[0])*(shape[1])*(shape[2]-1), 3))

idx_lui = idxs[lui_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]-1), 3))
idx_rdi = idxs[rdi_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]-1), 3))

idx_uui = idxs[uui_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]), 3))
idx_ddi = idxs[ddi_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]), 3))

idx_rui = idxs[rui_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]-1), 3))
idx_ldi = idxs[ldi_crop].reshape(((shape[0]-1)*(shape[1]-1)*(shape[2]-1), 3))

idx_lli = idxs[lli_crop].reshape(((shape[0]-1)*(shape[1])*(shape[2]-1), 3))
idx_rri = idxs[rri_crop].reshape(((shape[0]-1)*(shape[1])*(shape[2]-1), 3))

idx_iii = idxs[iii_crop].reshape(((shape[0]-1)*(shape[1])*(shape[2]), 3))
```
...
```{python}
def calc_edges3d(arr1, arr2, crop, index1, index2):
    weights = 1 - weight_of_arr(arr1, arr2)
    weight = weights[crop].ravel()
    idx1 = [(i, j, k) for i, j, k in index1[weight<0.7]]
    idx2 = [(i, j, k) for i, j, k in index2[weight<0.7]]
    return zip(idx1, idx2, weight[weight<0.7])
```

```{python}
# %time G3.add_weighted_edges_from(calc_edges3d(arr, ooo, ooo_crop, idx_ooo, idx_iii))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, iii, iii_crop, idx_iii, idx_ooo))

del idx_ooo
del idx_iii


# %time G3.add_weighted_edges_from(calc_edges3d(arr, luo, luo_crop, idx_luo, idx_rdi))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, rdi, rdi_crop, idx_rdi, idx_luo))

del idx_rdi
del idx_luo

# %time G3.add_weighted_edges_from(calc_edges3d(arr, uuo, uuo_crop, idx_uuo, idx_ddi))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, ddi, ddi_crop, idx_ddi, idx_uuo))

del idx_ddi
del idx_uuo

# %time G3.add_weighted_edges_from(calc_edges3d(arr, ruo, ruo_crop, idx_ruo, idx_ldi))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, ldi, ldi_crop, idx_ldi, idx_ruo))

del idx_ldi
del idx_ruo


# %time G3.add_weighted_edges_from(calc_edges3d(arr, llo, llo_crop, idx_llo, idx_rri))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, rri, rri_crop, idx_rri, idx_llo))

del idx_rri
del idx_llo


# %time G3.add_weighted_edges_from(calc_edges3d(arr, ldo, ldo_crop, idx_ldo, idx_rui))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, rui, rui_crop, idx_rui, idx_ldo))

del idx_rui
del idx_ldo


# %time G3.add_weighted_edges_from(calc_edges3d(arr, ddo, ddo_crop, idx_ddo, idx_ddo))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, uui, uui_crop, idx_uui, idx_uui))

del idx_uui
del idx_ddo


# %time G3.add_weighted_edges_from(calc_edges3d(arr, rdo, rdo_crop, idx_rdo, idx_lui))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, lui, lui_crop, idx_lui, idx_rdo))

del idx_lui
del idx_rdo


# %time G3.add_weighted_edges_from(calc_edges3d(arr, luu, luu_crop, idx_luu, idx_rdd))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, rdd, rdd_crop, idx_rdd, idx_luu))

del idx_luu
del idx_rdd


# %time G3.add_weighted_edges_from(calc_edges3d(arr, uuu, uuu_crop, idx_uuu, idx_ddd))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, ddd, ddd_crop, idx_ddd, idx_uuu))

del idx_uuu
del idx_ddd


# %time G3.add_weighted_edges_from(calc_edges3d(arr, ruu, ruu_crop, idx_ruu, idx_ldd))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, ldd, ldd_crop, idx_ldd, idx_ruu))

del idx_ruu
del idx_ldd


# %time G3.add_weighted_edges_from(calc_edges3d(arr, lll, lll_crop, idx_lll, idx_rrr))
# # %time G3.add_weighted_edges_from(calc_edges3d(arr, rrr, rrr_crop, idx_rrr, idx_lll))
...
```{python}
nx.write_weighted_edgelist(G3, '3d_graph')
```

```{python}
path2 = nx.shortest_path(G3, (39, 126, 96), (48, 115, 172), weight='weight')
```
...
```{python}
xyz = draw_nodes(G3, nodes3, path2)
```

```{python}
w.add_points(xyz, size=1)
```
...
```{python}
cutoff = np.percentile(node_dists, 99)
cutoff
```
...
```{python}
paths_merged = sorted(merge_paths(paths))
```
...
```{python}
w.add_points(draw_nodes(G3, nodes3, paths_merged), size=1, edge_color='none', face_color='m')
```

```{python}
while len(w.layers) >= 10: 
    w.layers.pop()
```
