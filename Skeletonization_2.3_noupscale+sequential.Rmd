...
def calc_edges(U, V, index1, index2, alpha=0.1, beta=0.001,
    ...
    if as_similarities:
        W = N*Sh*Se # temporarily, make weights as adjacency, not distance...
    else:
        W = Sx * beta + offset - N*Sh*Se
...
```{python}
g1_d = graph_dist.subgraph(comps_d[np.argmax(clengths_d)])
...
```{python}
w_d = napari.view_image(-fvx_d*(fvx_d<0),colormap='cyan', name='dist,neg')
w_d.add_image(fvx_d*(fvx_d>0), colormap='magenta',blending='additive',name='dist, pos')
```

```{python}
w_s = napari.view_image(-fvx_s*(fvx_s<0),colormap='cyan', name='sim,neg')
...
```{python}
w = napari.view_image(dv_d, colormap='inferno', name='degree, dist')
...
```{python}
np.min(eigvals_d.real), np.min(eigvals_s.real)
```
...
```{python}
plt.plot(eigvals_d, 'sb-')
plt.plot(eigvals_s, 'sr-')
```
...
```{python}
fve_s = np.zeros(final_image.shape, np.float32)


keig = 3
for vi, n in zip(tqdm(eigvecs_s[:,keig]), g1_s.nodes):
    fve_s[n] = vi
```
...
```{python}
w = napari.view_image(-fve_s*(fve_s<0),colormap='cyan', name='sim, neg',)
...
```{python}
clust = cluster.KMeans(n_clusters=20)
```

```{python}
labs_s = clust.fit_predict(eigvecs_s[:,1:])
labs_d = clust.fit_predict(eigvecs_d[:,1:])
```
...
```{python}
fvl_d = np.zeros(final_image.shape, np.uint8)
fvl_s = np.zeros(final_image.shape, np.uint8)

for vi, n in zip(tqdm(labs_s), g1_s.nodes):
    fvl_s[n] = vi + 1
    
for vi, n in zip(tqdm(labs_d), g1_d.nodes):
    fvl_d[n] = vi + 1    
```
