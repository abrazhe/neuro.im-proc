...
```{python}
#pd.read_csv(data_path.joinpath('avf-pic-correspondense.csv'))
```
...
def prune_tip(G, tip, min_length=10, max_nodes = 1000000):
    visited = {tip}
    acc = [tip]
    for i in range(max_nodes):
        parents = [p for p in G.predecessors(tip) if not p in visited]
        if not len(parents):
            break
        p = parents[0]
...
```{python}
if verbose:
    wf =  napari.view_image(final_image, ndisplay=3, opacity=0.5, blending='additive', scale=scale)    
    vis.view_graph_as_colored_image(gx_all_occ, final_image.shape, wf,scale=scale);
```
...
```{python}
#gx_all_occ.remove_node((29, 303, 282))
```

```{python}
tips = list(gu.get_tips(gx_all_occ))
```

```{python}
to_prune = [prune_tip(gx_all_occ, t) for t in tips]
```
...
```{python}
to_prune_all = reduce(op_.or_, to_prune)
```
...
```{python}
(29, 303, 282) in to_prune_all
```

```{python}
gx_all_occ2 = gx_all_occ.subgraph(n for n in gx_all_occ if n not in to_prune_all)
gx_all_occ2a = gx_all_occ.subgraph(n for n in gx_all_occ if n in to_prune_all)
```

```{python}
if verbose:
    wf =  napari.view_image(final_image, ndisplay=3, opacity=0.5, blending='additive', scale=scale)    
    wf.add_image(sigma_mask, colormap='turbo',scale=scale)
    vis.view_graph_as_colored_image(gx_all_occ2, final_image.shape, wf,scale=scale);
    #vis.view_graph_as_colored_image(gx_all_occ2a, final_image.shape, wf,scale=scale);
```
...
## Разбиение области мелких отростков на субдомены (lobes)
...
```{python}
paths = vis.graph_to_paths(gx_all_occ2)
```
...
```{python}
trees = [sp.KDTree(segment) for path_group in tqdm(paths2) for segment in path_group]
```
...
```{python}
thin_mask = (sigma_mask <=1) * (final_image > 0)
```

```{python}
w = napari.view_image(final_image, scale=scale)
w.add_image(sigma_mask > 1, colormap='red', blending='additive', visible=True,scale=scale)
#w.add_image(final_image >0, colormap='cyan', blending='additive', visible=True)
w.add_image(thin_mask, colormap='magenta', blending='additive', visible=True,scale=scale)
```

```{python}
pts = np.vstack(np.where(thin_mask)).T
```
...
```{python}
thin_tree = sp.KDTree(pts)
```

```{python}
# %time x = thin_tree.query(trees[0].data)
```

```{python}
# %time x1 = trees[0].query(thin_tree.data)
```

```{python}
x1[0].shape, x1[1].shape
```
...
```{python}
# %%time 
x1 = [tree.query(thin_tree.data)[0] for tree in tqdm(trees)]
```
...
```{python}
idx = x1.argmin(axis=0) + 1
idx.shape
```

```{python}
domains = np.zeros(final_image.shape, np.uint16)
domains_colored = np.zeros(final_image.shape + (3,), np.float32)
```

```{python}
domain_colors = np.random.rand(len(trees),3)
```

```{python}
for kc, loc in zip(idx, tqdm(pts)):
    loc = tuple(loc)
    domains[loc] = kc
    domains_colored[loc] = domain_colors[kc-1]
```

```{python}
w = napari.view_image(final_image, scale=scale)
w.add_image(domains_colored,opacity=0.75,channel_axis=3,scale=scale)
w.add_image(domains==542,opacity=0.75,colormap='red',scale=scale)
```

```{python}
#w.add_image(domains[31])
```
...
```{python}
np.save(data_path.joinpath(base_name+'small-domains.npy'),domains)
```
...
```{python}
@jit
def nullify_boundary_pixels(segmented, prob=1):
    out = segmented.copy()
    #seps = np.zeros(segmented.shape, np.uint8)
    #count = 0
    for loc in np.ndindex(segmented.shape):
        if segmented[loc] == 0:
            continue
        this = segmented[loc]
        for i in range(loc[0]-1,loc[0]+2):
            for j in range(loc[1]-1,loc[1]+2):
                for k in range(loc[2]-1,loc[2]+2):
                    other = segmented[i,j,k]
                    if other == 0:
                        continue
                    elif other > this and (np.random.rand() < prob):
                        out[loc] = 0
    return out
```

```{python}
domains_sep = nullify_boundary_pixels(domains, prob=0.99)
```

```{python}
seps = (domains_sep == 0) & (domains > 0)
```

```{python}
#np.sum(nsep), np.sum(seps)
```

```{python}
w = napari.view_image(final_image)
#w.add_image(domains_colored,opacity=0.75,channel_axis=3)
#w.add_image(domains_sep,opacity=0.75,colormap='green')
w.add_image(seps,opacity=0.75,colormap='red')
w.add_image(sigma_mask > 1, colormap='cyan', blending='additive', visible=True)
```
...
```{python}
sep_probs = [0, 0.05, 0.1, 0.2, 0.4, 0.8, 1.0]
```

```{python}
for prob in sep_probs:
    domains_sep = nullify_boundary_pixels(domains, prob=0.05)
    seps = (domains_sep == 0) & (domains > 0)
    np.save(data_path.joinpath(base_name+r'small-domains-pzero={prob}.npy'),domains_sep)
    np.save(data_path.joinpath(base_name+r'small-domains-pzero={prob}-perforates.npy'),seps)
```
...
```{python}
stack_cropped = np.array([crop_image(plane,domain_mask, margin=10) for plane in stack])
stack_croppedz = ndi.zoom(stack_cropped.astype(np.float32), (1, 1/downscale, 1/downscale))
stack_croppedz *= (final_image>0)
```

```{python}
w = napari.view_image(final_image,scale=scale)
w.add_image(stack_croppedz,scale=scale)
```

### Создаем шаблон для модели

```{python}
def make_sponge_plus_avf(im, cmask=None,pmin=0.1, pmax=99.9):
    if cmask is None:
        cmask = np.ones_like(im,dtype=np.bool)
    
    avf = ndi.gaussian_filter(im, 0.5)
    mn,mx = np.percentile(avf[cmask], (0.1, pmax))
    
    avf = (avf - mn)/(mx-mn)
    
    avf[~cmask] = 0
    outx = np.zeros(im.shape + (3,))
    #foreground_mask = dstack([foreground_mask]*3)
    outx[cmask,2] = 1
    outx[...,0] = avf
    outx[~cmask,:] = 0
    return np.clip(outx,0,1)

def estimate_noise_sigma(data, is_rectified=False):
    alpha = 0.5838
    nd = np.ndim(data)
    details = data
    for ax in range(nd):
        details = np.diff(details, axis=ax)
    correction = 2**(0.5*nd)
    if is_rectified:
        correction *= alpha
    
    return np.std(details)/correction
```
...
# %time stack_avf = make_sponge_plus_avf(np.maximum(0,stack_croppedz)**gamma, final_image > 0, pmax=99) 
```
...
```{python}
ui.group_maps(stack_avf, 10, colorbar=False)
plt.tight_layout()
```
...
```{python}
export_name = f'/home/brazhe/src/ageom-astrocytes/astroVolume3D/r3d_avf{kcell:02d}:{nz:02d}_planes-domains0.png'
export_name
```
...
```{python}
sep_probs = [0, 0.01, 0.05, 0.1, 0.2, 0.4, 0.8, 1.0]
#sep_probs = [0.01]
```
...
```{python}
export_path = '/home/brazhe/src/ageom-astrocytes/astroVolume3D/'

for prob in tqdm(sep_probs):
    domains_sep = nullify_boundary_pixels(domains, prob=prob)
    seps = (domains_sep == 0) & (domains > 0)
    nzeroed = np.sum(seps)
    print(prob, nzeroed)
    
    
    tmp_avf = stack_avf.copy()
    tmp_avf[seps] = 0
    rolled_avf = np.zeros((nr, nz*nc, 3))
    for i, p in enumerate(tmp_avf):
        rolled_avf[:,i*nc:(i+1)*nc] = p
    
    export_name1 = f'r3d_avf{kcell:02d}:{nz:02d}_planes-domains-pzero-{prob}.png'
    export_name2 = f'r3d_avf{kcell:02d}:{nz:02d}_planes-domains-pzero-{prob}-rand.png'
    
    plt.imsave(export_path + export_name1, rolled_avf)
    
    
    tmp_avf = stack_avf.copy()
    rand_pts = np.random.permutation(pts)[:nzeroed]
    ...
    plt.imsave(export_path + export_name2, rolled_avf)
    
    np.save(data_path.joinpath(base_name+f'small-domains-pzero={prob}.npy'),domains_sep)
    np.save(data_path.joinpath(base_name+f'small-domains-pzero={prob}-perforates.npy'),seps)
