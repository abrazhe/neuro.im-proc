...
```{python}
w.add_image(dilation(eroded), opacity=0.5, colormap='green', gamma=0.2)
```
...
```{python}
paths = nx.all_shortest_paths(G, (3, 56), (23, 28), weight='weight')
```
...
for path in tqdm(paths):
    ...
    for n in path:
        if cnode == n:
            print('.')
            continue
        elif n in cnode.children:
            cnode = cnode.children[n]
        else:
            new_node = at.Node(n, parent=cnode)
            cnode = new_node
```
...
for path in paths[::50]:
    w.add_shapes(draw_nodes(G3, nodes3, path[:-1]), shape_type='path', edge_color='red', edge_width=0.1)
```
